...
1. Write your bcftools command to select variants. **(10pts)**
2. Return the chromosome and position of the resulting variants. **(5pts)**
3. What genes do the variants fall in? **(5pts)**
4. Search for the exact genomic coordinates of these variants on the ClinVar website. What is the specific molecular consequence of each mutation (e.g., missense, nonsense/stop-gain, frameshift)? **(10pts)**
5. Why did we filter out variants with a gnomAD AF > 1%? **(10pts)**
...
# It only prints the first time it sees a coordinate, gracefully dropping redundant transcripts.
awk '!seen[$1,$2]++' step1_filtered_transcripts.txt > step2_deduplicated.txt

# Step 3: Sort the results mathematically by the CADD score.
...
# Step 4: Clean up the intermediate files to keep your directory tidy.
...
Your filtered ClinVar list missed a critical mutation on Chromosome 13. Let's use our in silico predictions to find it.

Use grep on your in_silico_predictions.txt to search specifically for variants in the BRCA2 gene. Locate the variant at chr13:32339132.

What is the functional consequence of this mutation (e.g., missense_variant, stop_gained, frameshift_variant)?

The Algorithm Trap: Look closely at the VEP string for this variant. You will notice that it has a high CADD score, but SIFT and PolyPhen failed to evaluate it (often represented by empty fields or lacking the "deleterious" tag). Based on the functional consequence of the BRCA2 variant you found in Question 2, explain biologically why SIFT and PolyPhen algorithms cannot score this specific mutation.

Clinical Conclusion: Look at the Phred-scaled CADD score for the BRCA2 variant. Based on this score and the biological function of the BRCA2 gene (a tumor suppressor that repairs DNA), explain why this is a high-confidence cancer driver despite missing a ClinVar "Pathogenic" classification.

::: -->


<!-- 1. Chromosome 17 (The absolute highest yield)

Genes: TP53 and BRCA1

What the students will find: HCC1395 is a fantastic teaching model because it contains both a nonsense germline variant in the BRCA1 gene and hotspot somatic mutations in the TP53 gene. The BRCA1 variant is a known nonsense mutation, specifically R1751X.

Pedagogical value: By giving students just Chromosome 17, you can teach them the critical bioinformatics difference between a germline cancer predisposition variant (which will have a ~50% variant allele frequency in both the normal and tumor VCF) and a somatic driver mutation (which will only be present in the tumor VCF).
...
What the students will find: The SEQC2 benchmark data confirms that HCC1395 also contains a somatic nonsense mutation in the BRCA2 gene.

3. Chromosome 8

Gene: FGFR1

What the students will find: There are known hotspot somatic mutations in the FGFR1 gene in this specific cell line.

4. Chromosome 3 and Chromosome 10

Genes: PIK3CA (Chr 3) and PTEN (Chr 10)

Note for teaching: While these are canonical breast cancer genes, they are not the primary drivers in HCC1395. If you want students to find aggressive, clear-cut mutations, stick to the BRCA and TP53 pathways for this specific dataset.

My Recommendation for the Classroom
Only have the students download and process the data for Chromosome 17 and Chromosome 13.

It drastically reduces the compute time and storage space required for the lab.

It perfectly sets up the workflow to find the "Big Three" breast cancer genes (BRCA1, BRCA2, and TP53).
...
## In silico score prediction

```sh
vep -i tumor_annotated.vcf.gz \
  -o final_clinical_tumor.vcf.gz \
  --vcf --compress_output bgzip \
  --sift b --polyphen b --plugin CADD,CADD.tsv.gz
```
...
4. Problem Set & Discussion Questions for Students
Here is a list of conceptual and practical problems you can assign based on this workflow:

The Frequency Paradox: You identify a variant with a high Variant Allele Frequency (VAF = 0.45) in the tumor sample, but your gnomAD annotation shows it has a population frequency of 0.30 (30%). Is this variant driving the cancer? Why or why not? (Goal: Distinguish between somatic drivers and common germline SNPs).
...
If they run the workflow correctly on the GRCh38 assembly of HCC1395, their final filtered list should prominently feature these three canonical mutations:

TP53 Somatic Hotspot Mutation (Chromosome 17)
...
Fact-Checking the FDA: The official FDA/SEQC2 consortium publication describing this cell line states that the tumor contains a BRCA2 somatic stop-gain mutation resulting from a G>A transversion. However, your VCF likely shows a different alternate allele at coordinate chr13:32339132.
