...
Use the VCF file that contains all the GnomAD and ClinVar annotations.
...
vep -i <input.vcf.gz> \
  ...
  --offline --cache \
  --symbol --canonical  \
  -sift b --polyphen b --plugin CADD,/project2/msalomon_1816/trgn_515/3_annotation/HCC1395_breast_cancer/cadd/whole_genome_SNVs.tsv.gz

# Index the output
tabix -p vcf <output.vcf.gz>
```
The VEP annotated VCF contains a massive amount of information in the CSQ string separated by pipes (|). We can use `bcftools query` to extract just the genomic coordinates, the Gene Symbol, and the predicted Consequence.

```sh
bcftools query \
  -f '%CHROM\t%POS\t%REF\t%ALT\t%INFO/CSQ\n' \
  HCC1395_vep_annotated.vcf.gz > vep_summary.txt
```
